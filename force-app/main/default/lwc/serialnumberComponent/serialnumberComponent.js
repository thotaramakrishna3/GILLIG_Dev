...
export default class SerialnumberComponent extends LightningElement {
  ...
  get returntrue(){
    ...
    @track existingseriallogvalue; //used to store previous serial number value
    //used to track change in serialnumber log
    existingserialonfocus(event) {
        this.existingseriallogvalue = event.target.value;
        console.log('entered existing serial ' + this.existingseriallogvalue);
    }

    async updateserialnumberlog(event) {
        console.log('entered new serial ' + event.target.value);
        var newseriallogvalue = event.target.value;
        if (this.existingseriallogvalue != newseriallogvalue) {
            console.log('serial number log changed');
            ...
            .then(data => {
                if(data.isError){
                    if (data.errorMessage == 202) {
                        let message = JSON.parse(data.responsebody).data.validation_message; // to correct msg on duplicate serial number and already modified case
                        const alertmessage = new ShowToastEvent({
                            //title: "Sorry we could not complete the operation.",
                            //title: "Duplicated Serial Number.",
                            title: "Failed to update.",
                            //message: "Couldn’t update data, "+JSON.parse(data.responsebody).data.validation_message,
                            message: message.charAt(0).toUpperCase()+message.substring(1, message.length),
                            variant: "error"
                        });
                        this.dispatchEvent(alertmessage);
                        this.loadSerialNologdata();
