...
export default class CustomMultiSelectCustomers extends LightningElement {
    ...
    handleComponentEvent(event){
        ...
        if(this.limittoone){
         ...
         var listSelectedItems =  this.lstSelectedRecords;
        var selectedAccountGetFromEvent = event.detail;
        const pillAddedEvent = new CustomEvent('pilladded', {
            detail: selectedAccountGetFromEvent
        });
        this.dispatchEvent(pillAddedEvent);
        listSelectedItems.push(selectedAccountGetFromEvent);
        this.lstSelectedRecords = listSelectedItems;
        } 
    ...
    clear(event){
        ...
        const pillRemovedEvent = new CustomEvent('pillremoved', {
            detail: selectedPillId
        });
        this.dispatchEvent(pillRemovedEvent);
    ...
    searchHelper(getInputkeyWord){
        ...
        if(getInputkeyWord == ''){
            ...
            //     var recordlower = record.label.toLowerCase();
            ...
            for (var item in listtosearchfor) {
                var customer = listtosearchfor[item];
                if (customer.label != null && customer.label != undefined) {
                    var recordlower = customer.label.toLowerCase();
                    if (recordlower.startsWith(searchStr)) {
                        if (storeResponse.length < 5) {
                            if (recordlower.match(searchStr)) {
                                const rec = customer;
        ...
        function checkselected(element, alreadyselectedrecords){
            ...
            for(var selected in alreadyselectedrecords){
                if(alreadyselectedrecords[selected].value == element.value){
                    isselected = true;
                }
            }
            return isselected;
