...
const pageNumber = 1;
const showIt = 'slds-visible';
const hideIt = 'slds-hidden'; //visibility keeps the component space, but display:none doesn't
...
export default class Paginator extends LightningElement {
    ...
    handleKeyChange() {
        if (this.debounceTimer) {
            clearTimeout(this.debounceTimer); // Clear the previous debounce timer
        }
        ...
        if(this.searchKeyWord){ 
                ...
                this.searchKey = this.searchKeyWord;
                    this.recordsToDisplay = this.records.filter(rec => JSON.stringify(rec).toLowerCase().includes(this.searchKeyWord.toLowerCase())).slice(0,500);
