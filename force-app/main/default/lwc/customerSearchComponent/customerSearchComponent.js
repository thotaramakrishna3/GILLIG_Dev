...
export default class CustomerSearchComponent extends LightningElement {
    ...
    onfocusmethod(event){
        if(!this.disableselection){ // newly added
     ...
     searchHelper(event, getInputkeyWord) {
        ...
        if(getInputkeyWord != undefined){
            ...
            for (var item in this.listtoSearch) {
                var customer = this.listtoSearch[item];
                if (customer != null && customer != undefined) {
                    var recordlower = customer.toLowerCase();
                    //if(recordlower.startsWith(searchStr)){
                    if (recordlower.includes(searchStr)) {
                        if (searchedList.length < 10) {
                            if (recordlower.match(searchStr)) {
