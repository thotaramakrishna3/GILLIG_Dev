...
export default class ShortageDbGilligPurchaseComponent extends LightningElement {
      ...
      .then(data => {
        ...
        var buyerlist = [];//
          ...
          for (var disc in shortagelogs) {
            ...
            if (shortageobj.buyer != null && shortageobj.buyer != '') {
  ...
  createbuyerpicklist(buyerlist) {
    var allbuyerslist = this.allbuyerslist.concat(buyerlist); // concate old and new buyers list
    var buyersset = new Set(allbuyerslist); //create set for the concatinated buyerlist to remove the duplicate
    var uniquebuyerslist = Array.from(buyersset).sort(); // set to array transform
    this.allbuyerslist = uniquebuyerslist; //store the new set of buyers to array
    var picklist = [{ label: "All Buyer", value: "All Buyer" }];
    for (var item in uniquebuyerslist) {
      var option = {
        "label": uniquebuyerslist[item],
        "value": uniquebuyerslist[item]
      };
