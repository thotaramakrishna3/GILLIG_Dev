  ...
  updateuserbutton(event){
    ...
    if (allValid && validcustomer) {
      ...
      if (this.record.approle_id == '6') {
        ...
        var customeridlist = [];
        if (Array.isArray(this.record.customer_id)) { // Check if customer_id is an array
          for (var i = 0; i < this.record.customer_id.length; i++) {
            const val = this.record.customer_id[i].value;
            if(!isNaN(val)){
              customeridlist.push(parseFloat(this.record.customer_id[i].value));
            }else{
              customeridlist.push(parseFloat(this.record.customer_id[i])); 
            }
          }
        } else { // If it's not an array, handle single value
          customeridlist.push(parseFloat(this.record.customer_id));
        }
