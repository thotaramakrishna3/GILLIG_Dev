...
import getRejectCount from '@salesforce/apex/GetRejectList.getRejectCount';
import getReworkCount from '@salesforce/apex/GetRejectList.getReworkCount';
              ...
              .then(data => {
                  if(data.isError){
                    ...
                    const alertmessage = new ShowToastEvent({
                      ...
                      message : 'The Ecard VIN Label successfully updated.',
                     variant : 'success'
                });
                this.dispatchEvent(alertmessage);
...
@track reworkCount;
  ...
  connectedCallback() {
    this.loadview();
    //refreshApex(this.rejectCount);
  
  }
  getEcardRejectCount(event) {
   //refreshApex(this.ecardid);
    var EcardId = this.ecardid;
    getRejectCount({ ecardId: EcardId })
      .then((data) => {
        if (data.isError) {
          const alertmessage = new ShowToastEvent({
  ...
  getEcardReworkCount(event) {
    var EcardId = this.ecardid;
    getReworkCount({ ecardId: EcardId })
      .then((data) => {
        if (data.isError) {
          const alertmessage = new ShowToastEvent({
            title: "Failed to fetch E Card Details.",
  ...
  showbusDetails(event) {
    ...
    this.getEcardRejectCount(event);
    this.getEcardReworkCount(event);
    //refreshApex(this.wiredRejectCount);
    this.loadview();
  ...
  updateecarddetailstoserver(event){
    var requestbody = {
      ...
      "build_date__c":this.ecarddetails.build_date__c
    };
              ...
              .then(data => {
                  if(data.isError){
                    ...
                    const alertmessage = new ShowToastEvent({
                      ...
                      message : 'The ecard detail was successfully updated.',
                     variant : 'success'
                });
                this.dispatchEvent(alertmessage);
  ...
  showfiltereddata(event){
    ...
    console.log('Label>>>'+viewtobeselected + '   ID>>>'+statustobefiltered);
    this.showbusoverview = false;
