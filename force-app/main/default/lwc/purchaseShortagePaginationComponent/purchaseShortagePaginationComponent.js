...
import groupupdatePartshortage from "@salesforce/apex/ecardOperationsController.updateGroupPartShortage";
...
import getAllShortageslist from "@salesforce/apex/ShortagesDatabaseController.getPurchaseShortagePaginationData";
...
import getBuyeranPlannerList from '@salesforce/apex/ShortagesDatabaseController.getBuyerandPlannerList'
export default class PurchaseShortagePaginationComponent extends LightningElement {
            ...
            .then(data => {
                ...
                for (var disc in shortagelogs) {
                    ...
                    var moddedshortage = {
                        ...
                        chassisno_with_basefleetno: `${shortageobj.chassis_no} / ${shortageobj.base_serial_fleet}`,
                        ...
                        customernamewithseq: customernamewithseq,
                        ...
                        vendor_to_sort: shortageobj.vendor_name == null ? "" : shortageobj.vendor_name,
                        remarks_sort: shortageobj.remarks == null ? "" : shortageobj.remarks.toLowerCase(),
                ...
                for (var i in shortageslist) {
                    ...
                    if (this.shortgecauselist.length > 1 && shortageslist[i].shortage_cause_id != null) {
                        for (var entry in this.shortgecauselist) {
                            if (shortageslist[i].shortage_cause_id == this.shortgecauselist[entry].value) {
                                shortageslist[i]['shortage_cause_value'] = this.shortgecauselist[entry].label;
                            }
                        }
                    }
                ...
                const sortColumn = this.previousColumn;
                if (sortColumn) {
                    const element = this.template.querySelector(
                        '[data-id="' + sortColumn + '"]'
                    );
                    if (!this.scrolltoTop) {
                        this.sortdiscrepancytable(element, 'function');
                    }
                    else {
                        element.iconName = '';
                    }
    ...
    sortdiscrepancytable(event, source) {
        ...
        if (source != 'function') {
            ...
            element = event;
            this.sortedColumn = previousSorted;
        }

        if (this.previousColumn != undefined) {
            var reverse = this.sortedDirection === "asc" ? 1 : -1;
            let table = JSON.parse(JSON.stringify(this.modifiedshortageslist));
            table.sort((a, b) => {
                return a[this.sortedColumn] >
                    b[this.sortedColumn]
                    ? 1 * reverse
                    : -1 * reverse;
            });
            ...
            if (this.sortedDirection === "asc") {
                element.iconName = "utility:chevronup";
            }
    ...
    handleshortgstatuschange(event) {
        ...
        this.handleUncheckSelected();
        this.applyfilter();
    }
 
    @track allbuyervalues = ["All Buyer"];  
    @track allSelectedbuyer = [];
    @track finalBuyerList = [];
    @track listOfBuyers;
    @track latestBuyerValue;
    handlebuyerchange(event) {
            this.allSelectedbuyer = event.target.value;
          
            if (this.allSelectedbuyer === "All Buyer") {
              this.allbuyervalues = ["All Buyer"];
            }
            else {
              if (!this.allbuyervalues.includes(this.allSelectedbuyer)) {
                this.allbuyervalues.push(this.allSelectedbuyer);
              }
              const allBuyerIndex = this.allbuyervalues.indexOf("All Buyer");
              if (allBuyerIndex !== -1) {
                this.allbuyervalues.splice(allBuyerIndex, 1);
              }
            }
            this.allSelectedbuyer = JSON.stringify(this.allbuyervalues);
            this.listOfBuyers = this.allbuyervalues;
            this.finalBuyerList = this.listOfBuyers.map(str => encodeURIComponent(str));
            this.latestBuyerValue = this.allbuyervalues[this.allbuyervalues.length - 1];
            this.applyfilter();
          }
          
    handlebuyerremove(event){
        const valueRemoved = event.target.name;
        this.allbuyervalues.splice( this.allbuyervalues.indexOf(valueRemoved), 1);
        if(this.allbuyervalues.length == 0){
            this.allbuyervalues =  ["All Buyer"];
        }
        this.latestBuyerValue = this.allbuyervalues[this.allbuyervalues.length - 1];
        this.finalBuyerList = this.allbuyervalues.map(str => encodeURIComponent(str));
        this.applyfilter();
    }
    ...
    //disable remark, vendor, causelist field update
    get disablepurchfieldedit() {
        return this.permissionset.shortage_update.write != true;
        ...
        for (var i in this.modifiedshortageslist) {
            if (this.modifiedshortageslist[i].ecard_discrepancy_log_id == selectepartshortagelogid) {
                ...
                if(this.modifiedshortageslist[i].resolved_status_updatedby_id !=null && (this.modifiedshortageslist[i].discrepancy_status == 'resolve' || this.modifiedshortageslist[i].discrepancy_status == 'approve')){
                    this.selectedshortage.resolved_updatedby_id = this.modifiedshortageslist[i].resolved_status_updatedby_id[0].fullname;
                }else{
                ...
                if(this.modifiedshortageslist[i].verifiedby_id !=null && this.modifiedshortageslist[i].discrepancy_status == 'approve'){
                    this.selectedshortage.verified_updatedby_id = this.modifiedshortageslist[i].verifiedby_id[0].fullname;
                }else{
    ...
    updatepartshortagetoserver(event) {
     const customSearchLookup = this.template.querySelector("c-custom-search-lookup-component");
     if(this.selectedshortage.date_received != null){
        if (!customSearchLookup || !customSearchLookup.selectedcustomer || this.selectedshortage.shortage_cause_id == null) {
            const alertmessage = new ShowToastEvent({
                title: "Vendor and Cause fields are mandatory when the Date Received field is filled.",
        ...
        const allValid = [
            ...this.template.querySelectorAll(".shipshortvalid")
        ...
        if (allValid) {
            ...
            var oldselectedshortage;
            for (var i in this.modifiedshortageslist) {
                if (this.modifiedshortageslist[i].ecard_discrepancy_log_id == discrepancytobeupdated.ecard_discrepancy_log_id) {
                    oldselectedshortage = Object.assign({}, this.modifiedshortageslist[i]);
                }
            }
            var part_shortage;
            // newly added by sathish for the auto resolve func
            if(discrepancytobeupdated.date_received != null && discrepancytobeupdated.is_ship_short == true && discrepancytobeupdated.discrepancy_status != 'resolve'){ 
                discrepancytobeupdated.discrepancy_status = 'resolve';
                this.statusascomment = true; 
            }else if (discrepancytobeupdated.date_received == null && oldselectedshortage.is_ship_short == true && discrepancytobeupdated.is_ship_short == false && discrepancytobeupdated.discrepancy_status != 'open'){ 
                discrepancytobeupdated.discrepancy_status = 'open'; 
                this.statusascomment = true; 
            }
            else{
                discrepancytobeupdated.discrepancy_status = discrepancytobeupdated.discrepancy_status;
            }
            // end
            ...
            if (discrepancytobeupdated.buyer_code != null) {
                for (var item in this.buyerpicklist) {
                    if (this.buyerpicklist[item]['buyer'] == discrepancytobeupdated.buyer_code) { 
                        part_shortage['buyer'] = this.buyerpicklist[item]['buyer'];
                         part_shortage['planner_code'] = this.buyerpicklist[item]['plan_cd'];
                        break;
                    }
                }
            }
            else {
                part_shortage['buyer'] = null;
                part_shortage['planner_code'] = null;
            }
            ...
            var responsebody = {
                ...
                "cut_off_date": discrepancytobeupdated.cut_off_date,
    ...
    modifydate(date) {
        ...
        if (date != undefined) {
            var jsdate = new Date(date);
            return jsdate.getUTCFullYear() + "-" + (jsdate.getUTCMonth() + 1) + "-" + jsdate.getUTCDate() + " " + jsdate.getUTCHours() + ":" + jsdate.getUTCMinutes() + ":" + jsdate.getUTCSeconds(); /*+ "00:00:00";*/
        }
        return formatteddate;
    }
    ...
    applyfilter(event) {
        ...
        this.applypaginationfilter(event);
    }
    ...
    applypaginationfilter(event){
        ...
        var statusparm = {
            ...
            search: this.selectedCustomer == undefined ? "" : this.selectedCustomer.replaceAll(" ", "%20"),
            ...
            has_date_recieved: !this.datereceivedfilter == true ? null : (!this.datereceivedfilter).toString(),
            createdby_id: this.selectedcreatedbyuser == undefined ? null : this.selectedcreatedbyuser,
            partno_search: this.serachpartnumber == undefined ? null : this.serachpartnumber,
            vendor_name: this.serachvendorname == undefined ? null : this.serachvendorname.replaceAll(" ", "%20") 
        };

        if (this.allbuyervalues.includes("All Buyer")) {
            statusparm.buyer = null; 
        }
        else {
            statusparm.buyer = this.finalBuyerList;
        }
        ...
        getBuyerforFilterOptions()
            .then((result) => {
                ...
                if (this.buyerlist.length == 1) {
                    var buyerlist = [{ label: "All Buyer", value: "All Buyer" }, { label: "Unknown Buyer", value: "empty"}];
...
get disabledatereceiveedit() {
    const isStatusOpen = this.selectedshortage.discrepancy_status.toLowerCase() === "open";
    const isShipShort = this.selectedshortage.is_ship_short;
    const isCauseSelected = !!this.selectedshortage.shortage_cause_id;
    const isVendorSelected = !!this.selectedshortage.vendor_name;

    // Enable the "Date Received" field only when both Vendor and Cause are selected
    const isDateReceivedEnabled = isCauseSelected && isVendorSelected;

    return (
        this.permissionset.shortage_update.write !== true ||
        !isStatusOpen ||
        isShipShort ||
        !isDateReceivedEnabled
    );
}

    @track buyerpicklist = [];
    @track buyernamelist = [];
    @track uniqueBuyerNames =[];
    getbuyernamelist() {
        getBuyeranPlannerList()
            .then((response) => {
                var buyernamelist = [];
                this.buyerpicklist = JSON.parse(response.operationlogresponse).data;
                for (var item in this.buyerpicklist) {
                    buyernamelist.push(this.buyerpicklist[item]['buyer']);
                }
                this.buyernamelist = buyernamelist;
                // Get unique buyer value
                this.buyernamelist.forEach(item => {
                    const buyerName = item;
                    if (!this.uniqueBuyerNames.includes(buyerName)) {
                      this.uniqueBuyerNames.push(buyerName);
                    }
                  });
                  this.uniqueBuyerNames = this.uniqueBuyerNames;

            })
            .catch((error) => {
                const alertmessage = new ShowToastEvent({
                    title: "Failed to fetch Buyer / planner list.",
                    message:
                        "Something unexpected occured. Please contact your Administrator",
                    variant: "error"
                });
    ...
    handlebuyerselect(event) {
        this.selectedshortage.buyer_code = event.detail.selectedRecord;
        if (event.detail.incident == 'selection') {
            this.isupdated = true;
        }
    }

    handlebuyerclear(event) {
        this.selectedshortage.buyer_code = null;
        this.isupdated = true;
    }
  ...
  highlightRow(event) {
    ...
    @track serachpartnumber = '';
    @track oldPartSearch = '';

    handlepartnosearch(event) {
        var newSerachKey = event.target.value.trim();
        if (this.oldPartSearch != newSerachKey) {
            this.serachpartnumber = newSerachKey;
            this.applypaginationfilter(event);
        }
    }

    handleoldpartvalue(event) {
        this.oldPartSearch = event.target.value;
    }

    @track serachvendorname = '';
    @track oldVendorSearch = '';
                
    handleoldvendor(event) {
        this.oldVendorSearch = event.target.value;
    }
                
    handlevendorsearch(event) {
        var newSearch = event.target.value;
        if (this.oldVendorSearch != newSearch) {
            this.serachvendorname = newSearch;
            this.applypaginationfilter(event);
            this.applyfilter();
            this.getEcardandCustomerforSearch();
        }
     ...
     // Group Shortages Update
     @track selectedIds = [];
     @track allSelectedId =[];
     @track allSelectedIdArray = [];
     ...
     handleCheckboxChange(event) {
         ...
         const ecard_discrepancy_log_id = event.target.getAttribute('data-id');
     
         if (this.isChecked == true) {
             if (!this.selectedIds.includes(ecard_discrepancy_log_id)) {
                 if (this.selectedIds) {
                     this.selectedIds += ',' + ecard_discrepancy_log_id;
                 } else {
                     this.selectedIds = ecard_discrepancy_log_id;
                 }
             }
         } else {
             const selectedArray = this.selectedIds.split(',');
             const index = selectedArray.indexOf(ecard_discrepancy_log_id);
             if (index !== -1) {
                 selectedArray.splice(index, 1);
                 this.selectedIds = selectedArray.join(',');
                 this.selectedGroupshortage = {};
             }
         }
         this.allSelectedId = this.selectedIds.startsWith(',') ? this.selectedIds.slice(1) : this.selectedIds;
         this.allSelectedIdArray = this.allSelectedId.split(',');
         console.log('Select shortage Ids>>>'+this.allSelectedIdArray);
         this.allSelectedIdArray = this.allSelectedIdArray.map(item => item.trim());   
          
     }
 
     @track groupupdate = false;
     onclickGroupupdate(event){
         this.groupupdate = true;
        
     }
  
     hideGroupUpdate(event) {
         this.groupupdate = false;
         this.allSelectedIdArray = [];
         this.selectedGroupshortage = [];
         this.handleUncheckSelected();
     }
 
     get checkSelectId(){
         return this.allSelectedId.length != 0;
     }
 
     @track selectedGroupshortage = [];
     updateGroupselectedshortage(event) {
         this.isupdated = true;
         var targetvalue;
         ...
         var targetname = event.target.name;
         this.selectedGroupshortage[targetname] = targetvalue;
         this.updatedisable = false;
     }
 
     updategroupupdatestoServer(event){ 
             var responsebody = {
                  "ecard_discrepancy_log_id": this.allSelectedIdArray,
                  "modified_date": new Date().toISOString()
             };
             for (var key in this.selectedGroupshortage) {
                  if (this.selectedGroupshortage.hasOwnProperty(key) && this.selectedGroupshortage[key] !== null && this.selectedGroupshortage[key] !== undefined) {
                     responsebody[key] = this.selectedGroupshortage[key];
             ...
             console.log('Group update Shortages>>>'+JSON.stringify(responsebody));
            
             groupupdatePartshortage({ requestbody: JSON.stringify(responsebody) })           
                 .then(data => {
                     if (data.isError) {
                         if (data.errorMessage == 202) {
                             this.showmessage('Sorry we could not complete the operation.', JSON.parse(data.responsebody).data.validation_message, 'error');
                         ...
                         else if (data.errorMessage == 422) {
                             this.showmessage('PO Number, ETA, Carrier, Carrier Arrival Time, Tracking, Date Received cannot be updated for Resolved and Verified Shortages.', 
                                                 JSON.parse(data.responsebody).data.validation_message, 'error');
                             this.partshortagedetailsmodal = false;
                             this.loadShortagesdata(event, true);         
                         }
                         else {
